...
class Content extends Component {
    ...
    render () {
        ...
        return [
            <Title key="0" title="Spring的对象代理" />,

                    <Paragraph key="1" header="什么是代理" ps={
                        [
                                <p key='0'></p>,
                                <p key='1'>代理是指由一个主体代替另一个主体完成某些事情。代理的根本目的仍是完成被代理主体的任务。</p>,
                                <p key='2'></p>,
                                <p key='3'>从程序的角度来说，满足下面几个条件的情况下，我们就称A是B的代理：</p>,
<pre key='4'>
  <code className='' style={codeStyle}>
    {`    1. A和B对外提供一样的接口
...
class Proxy {
    ...
    public static void main(String args[]) {
        new Proxy(new Raw()).sayHello();
                                ...
                                <p key='8'>这个示例有一个问题。Proxy虽然可以代理Raw的sayHello行为，但是并不能在调用Raw的地方被使用（由于Java是静态类型语言）。所以在Java里我们一般通过实现被代理类的超接口，或者继承被代理类的方式来实现代理。</p>,
<pre key='9'>
  <code className='java' style={codeStyle}>
    {`// 被代理接口
...
class Main {
    ...
    public static void main(String args[]) {
        ...
        say(new ProxyC(new Raw())); // ok
                    ...
                    <Paragraph key="2" header="动态代理" ps={
                        [
                                <p key='0'>代理可以在不改变原有类的情况下添加逻辑，增强原有类的功能。但是如果以手写代码的方式实现代理，我们将需要为不同的被代理类实现同一代理逻辑的不同代理类版本，我们还要处理代理逻辑组合的问题。代理类将呈指数增长。所以我们一般使用动态生成代理的方式实现代理。</p>,
                                <p key='1'></p>,
                                <p key='2'>所谓动态代理就是在运行时动态生成代理。</p>,
                                <p key='3'></p>,
                                <p key='4'>现在假设我们希望记录方法执行的时间，我们可以针对Raw或者IRaw编写一个代理对象：</p>,
<pre key='5'>
  <code className='java' style={codeStyle}>
    ...
    public void sayHello () {
        ...
        System.out.println("$interface.$m.name Taking " + (System.currentTimeMillis() - start));
                                ...
                                <p key='6'>如果我们还有其它的几十个类几百个方法，我们肯定不想为每个类都编写一个TimerProxy类。我们可以使用反射的方式来生成代理类的代码，然后使用程序编译和加载生成的代理类（关于运行时编译代码和加载类的相关信息可以搜索JavaCompiler）。</p>,
<pre key='7'>
  <code className='java' style={codeStyle}>
    {`// 生成代理的模板
...
class ProxyCreator {
    ...
    public static void main(String args[]) {
        ...
        rawProxy.getConstructor(IRaw.class).newInstance(new Raw()).sayHello();
                                ...
                                <p key='8'>实际生产中我们使用直接生成字节码的方式生成代理类，不必生成代理类的代码再进编译。常用的代理生成技术有Java标准库提供的代理技术和Cglib库。</p>,
                                <p key='9'></p>,
                        ]
                    }/>,
                    <Paragraph key="3" header="Java标准库的动态代理" ps={
                        [
                                <p key='0'>Java标准库的动态代理是基于接口的，被代理类需要实现接口才能被代理。动态织入的代理逻辑需要实现InvocationHandler接口。</p>,
                                <p key='1'>下面是一个标准库动态代理使用的示例：</p>,
<pre key='2'>
  <code className='java' style={codeStyle}>
...
public final class $Proxy0 extends Proxy implements IRaw {
    ...
    static {
        m1 = Class.forName("com.songfeifan.test.spring.core.IRaw").getMethod("sayHello");
                    ...
                    <Paragraph key="4" header="Cglib动态代理" ps={
                        [
                                <p key='0'>Cglib动态代理使用继承和方法回调的方式实现，动态织入的代理逻辑需要实现MethodInterceptor接口。</p>,
                                <p key='1'>下面是一个Cglib库动态代理使用的示例：</p>,
<pre key='2'>
  <code className='java' style={codeStyle}>
    ...
    public final void sayHello() {
        ...
        if (mInterceptor != null) {
            ...
            super.sayHello();
                    ...
                    <Paragraph key="5" header="Spring的动态代理" ps={
                        [
                                <p key='0'>Spring依靠动态代理的技术为对象添加增强逻辑，实现面向切面的编程方式。Spring为被依赖的对象注入依赖对象的增强代理，实现增强功能的调用。换句话说，只有被Spring容器管理的对象才有可能使用Spring的增强功能，因为要使用增强功能，必须使用Spring生成的代理对象。</p>,
                                <p key='1'></p>,
                                <p key='2'>Spring使用Java标准库或者Cglib库实现动态代理。基本的选择逻辑如下：</p>,
<pre key='3'>
  <code className='' style={codeStyle}>
...
5. 可能存在的其它逻辑`}
                                ...
                                <p key='4'></p>,
                                <p key='5'>一般情况下，Spring使用组合的方式实现被代理类的方法调用。大致上逻辑可以视作：</p>,
<pre key='6'>
  <code className='java' style={codeStyle}>
    {`// 代理类的大概逻辑
class RawP extends Raw {
    ...
    public void sayHello() {
        ...
        System.out.pringln("sayHello proxied");
                                ...
                                <p key='7'></p>,
                                <p key='8'>使用这种方式，被代理类在方法体内调用自身的方法的时候，增强功能将会失效。</p>,
                                <p key='9'>我们假设Raw类还有一个sayHelloAgain方法，调用自身的sayHello方法。</p>,
<pre key='10'>
  <code className='java' style={codeStyle}>
    {`class Raw {
    ...
...
class RawP extends Raw {
    ...
    public static void main(String args[]) {
        ...
        raw.sayHelloAgain();
                                ...
                                <p key='11'></p>,
                                <p key='12'>当我们使用Java config的方式配置Spring时，我们需要被代理类在调用自身方法时是调用的代理类的方法。因为我们需要被@Bean标记的方法每次调用都返回同一个对象（如果是单例模式）。</p>,
<pre key='13'>
  <code className='java' style={codeStyle}>
    {`@Configuration
class SysConfiguration {
    ...
    public boolean check() {
        return raw() == raw(); // must be true
                                ...
                                <p key='14'>所以被@Configuration标记的类的代理使用继承的方式实现，使用子类方法会覆盖掉父类方法的特性。</p>,
<pre key='15'>
  <code className='java' style={codeStyle}>
    {`// 代理类
class RawP extends Raw {
    ...
    public static void main(String args[]) {
        ...
        raw.sayHelloAgain();
                    ...
                    <Paragraph key="6" header="为什么有时候Spring的增强功能不会起作用" ps={
                        [
                                <p key='0'>Spring的增强功能不起作用主要可以分为两种情况。第一种情况是增强功能没有被启用，比如启用缓存需要@EnableCaching注解（Spring boot）。第二种情况是增强功能虽然被启用，但是实际调用方法时没有使用代理类调用方法。</p>,
                                <p key='1'>没有使用代理类的一些典型情况：</p>,
<pre key='2'>
  <code className='' style={codeStyle}>
