    ...
    render () {
        ...
        return [
            ...
            <Paragraph key="1" header="什么是代理" ps={
                [
                    <p key="0">代理是指由一个主体代替另一个主体完成某些事情。代理的根本目的仍是完成被代理主体的任务。</p>,
                    <p key="1">从程序的角度来说，满足下面几个条件的情况下，我们就称A是B的代理：</p>,
                    <pre key="2" >
...
`}
                    ...
                    <p key="5">这个示例有一个问题。Proxy虽然可以代理Raw的sayHello行为，但是并不能在调用Raw的地方被使用（由于Java是静态类型语言）。所以在Java里我们一般通过实现被代理类的超接口，或者继承被代理类的方式来实现代理。</p>,
                    <pre key="6">
                        <code className="java" style={codeStyle}>
{`// 被代理接口
interface IRaw { 
...
// 调用示例
class Main {
    static void say(Raw  raw) { 
        raw.sayHello(); 
    }

    static void say(IRaw raw) { 
        raw.sayHello(); 
    }
        ...
        say(new ProxyC(new Raw())); // ok
            ...
            <Paragraph key="2" header="动态代理" ps={
                [
                    ...
                    <p key="1">所谓动态代理就是在运行时动态生成代理。</p>,
                    <p key="2">现在假设我们希望记录方法执行的时间，我们可以针对Raw或者IRaw编写一个代理对象：</p>,
                    <pre key="3" >
                        <code className="java" style={codeStyle}>
...
class ProxyCreator {
    ...
    public static void main(String args[]) {
        ...
        rawProxy.getConstructor(IRaw.class).newInstance(new Raw()).sayHello();
                    ...
                    <p key="6">
                        实际生产中我们使用直接生成字节码的方式生成代理类，不必生成代理类的代码再进编译。
                        常用的代理生成技术有Java标准库提供的代理技术和Cglib库。
                    </p>,
...
下面是一个Cglib库动态代理使用的示例：`}
            ...
            <Paragraph key="5" header="Spring的动态代理" ps={
                [
                    <p key="0">
                        Spring依靠动态代理的技术为对象添加增强逻辑，实现面向切面的编程方式。
                        Spring为被依赖的对象注入依赖对象的增强代理，实现增强功能的调用。
                        换句话说，只有被Spring容器管理的对象才有可能使用Spring的增强功能，因为要使用增强功能，必须使用Spring生成的代理对象。
                    </p>,
                    <p key="1">
...
class RawP extends Raw {
    ...
    public static void main(String args[]) {
        ...
        raw.sayHelloAgain();
                    ...
                    <p key="6">
                        当我们使用Java config的方式配置Spring时，我们需要被代理类在调用自身方法时是调用的代理类的方法。
                        因为我们需要被@Bean标记的方法每次调用都返回同一个对象（如果是单例模式）。
                    </p>,
                    <pre key="7">
                        <code className="java" style={codeStyle}>
{`@Configuration
